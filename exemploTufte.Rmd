---
title: "O uso da equação de Manning para definição de relações cota-vazão em locais de curvas-chave não unívocas"
subtitle: "Uma abordagem para implantação operacional"
author: "Marcus Suassuna Santos e Luna Gripp"
date: "`r Sys.Date()`"
output:
...
Curvas-chave, importância, dificuldades, curvas-chave não unívocas, etc.

Planície amazônica é repleta de exemplos: região planas, formação de igarapés, igapós, lagoas, grande amplitude dos níveis, etc.

Essa característica leva a problemas nas estimativas de vazões utilizando métodos convencionais para traçado de curvas-chave.

```{marginfigure, ccTradicional}
Curva-chave convencional:

$$Q(h)= a \left(h - h0 \right)^{n} $$
```


Modelos hidráulicos mais complexos podem ser utilizados para avaliações do comportamento hidráulico do canal fluvial, porém tem limitações para seu uso na rotina operacional de estações fluviométricas.

Uma possibilidade que permitiria satisfazer as necessidades operacionais, considerando outros aspectos hidráulicos, seria a utilização da Equação de Manning como alternativa. Ela permitiria considerar aspectos hidráulicos que são negligenciados por métodos convencionais de curva-chave, e permite também uma versão simplificada possível de ser posta em prática na rotina operacional de estações fluviométricas.

```{marginfigure, Manning}
Equação de Manning:

$$Q(h)=\frac{1}{n} A(h)^{\frac{5}{3}} P(h)^{\frac{-2}{3}} S^{\frac{1}{2}}$$
```
...
## Perfil transversal da estação de Manaus

```{r perfilManaus, fig.margin = TRUE, fig.cap = "Perfil transversal na seção de medição em Manaus", echo = FALSE, warning=FALSE, message=FALSE}
X <- Perfil$Dist
Y <- Perfil$Cota
plot(X, Y, type = "l", bty = "n", ylab = "Cota (m)", xlab = "Distância (m)")
```

Foram feitas medições com ADCP em Manaus. A seção transversal é apresentada ao lado. A curva que relaciona Cota e a Área da seção transversal é apresentada abaixo, junto com a equação de regressão.

```{r cotaArea, fig.cap = "Relação entre medições de cota e área", echo = FALSE, warning=FALSE, message=FALSE}
...
plot(Cotas, Areas, pch = 20, bty = "n", xlab = "Cota (m)", ylab = "Área (m²)", col = "darkgrey")
lines(Cotas, predict(fit, newdata = data.frame("Cotas" = Cotas, "X" = X)), col = 2)
text(16, 97000,
     paste0("Area = ", round(fit$coefficients[1]), " + ",
            round(fit$coefficients[2]), " * Cota + ",
            round(fit$coefficients[3], 3), " * Cota²"))
...
```{r PortoVelho_1, fig.margin = TRUE, fig.cap = "Pares Cota/Vazão e curva-chave em Porto Velho", echo = FALSE, warning=FALSE, message=FALSE}
X <- PV$Cota
Y <- PV$Vazao
fit <- lm(log(Y) ~ log(X/10))
plot(X/100, Y, bty = "n", pch = 20)

# Ajusta uma relação não-linear
power.nls.PV <- nlsLM(Y ~ a * (X - h0) ^ n,
                   start = list(a = fit$coefficients[1], n = fit$coefficients[2], h0 = -1))

lines((0:20), 
      predict(power.nls.PV, 
...
```{r Manaus_1, fig.margin = TRUE, fig.cap = "Pares Cota/Vazão e curva-chave em Manaus", echo = FALSE, warning=FALSE, message=FALSE}
X <- Manaus$cota
Y <- Manaus$vazao
...
# Ajusta uma relação não-linear
power.nls.MN <- nlsLM(Y ~ a * (X - h0) ^ n,
                   start = list(a = fit$coefficients[1], n = fit$coefficients[2], h0 = -1))

lines((0:30), 
      predict(power.nls.MN, 
              newdata = data.frame("X" = (0:30)*100)), col = "red", lwd = 4)

```


Ao lado são apresentados exemplos muitos distintos de pares Cota-Vazão observados na região Amazônica. Em Porto Velho, existe alguma dispersão dos dados em torno de uma tendência central, mas não da mesma forma que ocorre em Manaus. Os coeficiente da curva chave **a**, **n** e **ho** de Porto Velho foram estimados em `r round(summary(power.nls.PV)$coefficients[1,1], 3)`, `r round(summary(power.nls.PV)$coefficients[2,1], 3)` e `r round(summary(power.nls.PV)$coefficients[3,1], 3)`, respectivamente. O erro médio quadrático foi estimado em `r round(sqrt(mean(residuals(power.nls.PV)^2)))` m³/s. 

Ainda que possa ser considerado um erro médio elevado, esses erros são bastante reduzidos quando comparados àqueles estimados em Manaus. Os coeficiente da curva chave **a**, **n** e **ho** de Manaus foram estimados em `r round(summary(power.nls.MN)$coefficients[1,1], 4)`, `r round(summary(power.nls.MN)$coefficients[2,1], 3)` e `r round(summary(power.nls.MN)$coefficients[3,1], 3)`, respectivamente. O erro médio quadrático é significativamente mais elevado que o de Porto Velho, estimado em `r round(sqrt(mean(residuals(power.nls.MN)^2)))` m³/s. Comparados com as vazões médias das medições, os erros relativos de Manaus também são significativamente superiores, estimados em `r round(sqrt(mean(residuals(power.nls.MN)^2))/mean(Manaus$vazao) * 100,2)`% da vazão média, enquanto o de Porto Velho é estimado em `r round(sqrt(mean(residuals(power.nls.PV)^2))/mean(PV$Vazao) * 100, 2)`%.
